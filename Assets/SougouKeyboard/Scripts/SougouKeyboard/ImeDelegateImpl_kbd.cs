...
{
    public static bool isFocusOn = false;//焦点是否在键盘上
    //------------------Ilyas-------------------start
    ...
    //------------------Ilyas------------------end
    public InputField inputField;// Current focused InputField
    //public Text mText;
    ...
    {
        if (!string.IsNullOrEmpty(strCommit))
        {
            if (inputField != null && mManager != null)
            {
                inputField.text += strCommit;
            }
        }
    }
    ...
    public override void OnIMEKey(SGImeKey key)
    {
        switch (key)
        {
            case SGImeKey.KEYCODE_DEL:
                ...
                inputField.text = strText.Remove(strText.Length - 1);
                break;
    ...
    private void CheckMouseEvent()
    {
        if (Point2UV(Input.mousePosition, ref mPtKbd))
        {
            ...
            InputDevices.GetDeviceAtXRNode(XRNode.LeftHand).TryGetFeatureValue(CommonUsages.triggerButton, out inputvalue);
            if (!inputvalue)
                InputDevices.GetDeviceAtXRNode(XRNode.RightHand).TryGetFeatureValue(CommonUsages.triggerButton, out inputvalue);
    ...
    private bool Point2UV(Vector3 ptScreen, ref Vector2 ptUV)
    {
        ...
        if (CurrentRayInteractor != null && CurrentRayInteractor.TryGetCurrent3DRaycastHit(out raycastHit))
        {
